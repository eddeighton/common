...
void parse_impl_actual( boost::spirit_ext::ParseResult& parseResult, TheType& result, std::ostream& errorStream )                   \
{                                                                                                                                   \
    boost::spirit_ext::invoke_parser<                                                                                               \
        TheGrammar< boost::spirit_ext::LinePosIterator >,                                                                           \
        boost::spirit_ext::SkipGrammar< boost::spirit_ext::LinePosIterator >,                                                       \
        TheType >                                                                                                                   \
            ( parseResult, result, errorStream );                                                                                   \
}
...
namespace boost
{
    namespace spirit_ext
    {
        typedef std::string::const_iterator StdStringIterator;
        typedef boost::spirit::line_pos_iterator< StdStringIterator > LinePosIterator;

        struct ParseResult
        {
            template< class IteratorType >
            ParseResult( IteratorType iterStart, IteratorType iterEnd )
                :   bParseSucceeded( false ),
                    iterStart( iterStart ),
                    iterReached( iterStart ),
                    iterEnd( iterEnd )
            {
            }
            ...
            inline IterType getIterReached() const { return IterType( iterReached.base() ); }
        };
        ...
        ParseResult parse( InputIteratorType iterBegin, InputIteratorType iterEnd, ResultType& result, std::ostream& errorStream )
        {
        ...
        inline T parse( IterType iStart, IterType iEnd, std::ostream& osError )
        {
            ...
            VERIFY_RTE_MSG( result.bParseSucceeded && result.iterReached.base() == iEnd, "Failed to parse string: " << std::string( iStart, iEnd ) );
            return resultType;
        }
        ...
        {
        public:
            CommentGrammar() : CommentGrammar::base_type( start )
            {
        ...
        {
        public:
            NoSkipGrammar() : NoSkipGrammar::base_type( start )
            {
                start = !boost::spirit::ascii::char_;
            }
        ...
        public:
            ...
            {
                m_main_rule = *( boost::spirit::ascii::char_
        ...
        inline void strip( const std::string& strInput, std::string& strOutput )
        {
            ...
            invoke_parser< 
                AllGrammar< boost::spirit_ext::LinePosIterator >, 
